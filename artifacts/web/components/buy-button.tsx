...
export function BuyButton({
  ...
  const [term, setTerm] = useState<'monthly' | 'quarter' | 'biannual' | 'annual'>('monthly')
  const TERMS = [
    { v: 'monthly' as const, l: 'Monthly', save: '' },
    { v: 'quarter' as const, l: '3 months', save: 'save 15%' },
    { v: 'biannual' as const, l: '6 months', save: 'save 25%' },
    { v: 'annual' as const, l: '12 months', save: '2 months free · Founder' },
  ]
  ...
  return (
    <div>
      {terms && payload.tier && (
        <div className="mb-3 grid grid-cols-2 gap-1 rounded-2xl border border-neutral-700 p-1 sm:grid-cols-4 sm:rounded-full">
          {TERMS.map((t) => (
