...
function StageCell({ email, value }: { email: string | null; value: string }) {
  ...
  if (!email) return <span className="text-xs text-neutral-300">—</span>;
...
function Pill({ on, label }: { on: boolean; label: string }) {
  return <span className={`mr-1 rounded-full px-2 py-0.5 text-[11px] ${on ? "bg-emerald-500/15 text-emerald-400" : "bg-neutral-800 text-neutral-300"}`}>{label}</span>;
}
...
export function WinbackTable({ rows }: { rows: Row[] }) {
  ...
  if (rows.length === 0) return <p className="text-sm text-neutral-300">No cancellations yet.</p>;
