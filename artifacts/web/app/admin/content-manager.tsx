  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
          ...
          <div className="mt-4 space-y-3">
            ...
            {rows.map((row, i) => (
              <div key={row.id} className="rounded-lg border border-neutral-800 bg-neutral-950 p-3">
                <p className="mb-2 truncate text-xs text-neutral-300">{i + 1}. {row.file.name}</p>
                <input
                  value={row.title}
        ...
        <p className="mt-2 text-xs text-neutral-300">Up to 5 photos (or one video) per upload — each is published with its own title (auto-filled from the file name, fully editable). iPhone HEIC photos convert automatically. Visible to the selected tier and above for 4 weeks, then moves to the Vault automatically.</p>
      </div>

      <div>
        <p className="mb-2 text-sm font-medium">Published content ({items.length})</p>
        {items.length === 0 ? <p className="text-sm text-neutral-300">Nothing published yet.</p> : (
          <ul className="divide-y divide-neutral-800 rounded-xl border border-neutral-800">
            {items.slice(page * PER, page * PER + PER).map((it) => (
              <li key={it.id} className="flex items-center justify-between px-4 py-2 text-sm">
                <span>{it.title} · <span className="text-neutral-300">{tierName[it.min_tier] ?? it.min_tier}</span> · {it.live ? <span className="text-emerald-400">live</span> : <span className="text-neutral-300">vault</span>} · {it.published_at ? new Date(it.published_at).toLocaleDateString("en-GB") : "—"}</span>
