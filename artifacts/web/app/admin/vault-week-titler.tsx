...
export function VaultWeekTitler({ weeks }: { weeks: AdminWeek[] }) {
  if (weeks.length === 0) {
    return <p className="text-sm text-neutral-300">No archived weeks yet. Once content is older than the live 4-week window, its week appears here to title and sell.</p>;
  }
  return (
...
function Row({ week }: { week: AdminWeek }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-neutral-800 bg-neutral-900 p-2">
      <span className="text-xs text-neutral-300 w-40 shrink-0">{week.rangeLabel} · {week.count} item{week.count === 1 ? "" : "s"}</span>
