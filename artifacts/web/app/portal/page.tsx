...
export default async function PortalPage() {
  ...
  if (user) {
    const { data: profile } = await supabase
      ...
      .eq("id", user.id)
      .maybeSingle();
    rank = tierRank(profile?.membership_tier);
  }
        ...
        <header className="mb-10 text-center">
          ...
          <nav className="mt-6 flex justify-center gap-4 text-sm">
            ...
            {rank >= 2 ? (
              ...
              <Link href="/subscribe" className="opacity-40 hover:opacity-70">Private World 🔒</Link>
            )}
            {rank >= 3 ? (
              <Link href="/portal/all-access" className="hover:text-amber-500">All Access</Link>
            ) : (
              <Link href="/subscribe" className="opacity-40 hover:opacity-70">All Access 🔒</Link>
            )}
          </nav>
        </header>
